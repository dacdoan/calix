...
impl Store {
    ...
    /// [`Self::upsert_google_event`] for one expanded occurrence of a series,
    /// which takes its reminder from the series id rather than its own.
...
mod tests {
    ...
    #[test]
    fn an_upsert_the_local_edit_guard_skipped_still_reports_that_row() {
        // The guard makes the `DO UPDATE` conditional, so `RETURNING` hands
        // back nothing when it bites. The row is still the one this Google id
        // names, and undo asks for an event by the id its create reported.
        let store = Store::open_in_memory().unwrap();
        let account_id = store
            .upsert_google_account("me@example.com", "me@example.com", "token-key")
            .unwrap();
        ...
        let protected = store
            .upsert_google_event(
                calendar_id,
                "evt-1",
                &draft("Standup (from server)", start, end),
                &[],
            )
            .expect("the upsert to run against a protected row");

        assert_eq!(protected, id, "a protected row still names itself");
        assert_eq!(
            store.event_by_id(id).unwrap().map(|event| event.title),
            Some("Standup".to_string()),
